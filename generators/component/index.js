    ...
    writing () {
        ...
        if (this.answers.backConf) {
            ...
            for (var i = 0; i < that.settings.backComponents.length; i++) {
                insert+= "\n\tmodels.createModel(require('./" + that.settings.backComponents[i] + "/" + that.settings.backComponents[i] + ".model.js'))";
            }
            ...
            insert = "";
            for (var i = 0; i < that.settings.backComponents.length; i++) {
                insert+= "\n\trouter.use('/" + that.settings.backComponents[i].toLowerCase() + "', require('./" + that.settings.backComponents[i] + "/" + that.settings.backComponents[i] + ".route.js')(app))";
            }
            newData = [newData.slice(0, start), insert + "\n", "\t" + newData.slice(start)].join('');
            fs.writeFileSync('app/backend/index.js', newData, 'utf-8');
